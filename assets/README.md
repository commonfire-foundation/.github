...
This public `.github` repository is the canonical home for CommonFIRE brand assets. The organization profile uses the light/dark banners from `banners/`, with URLs pinned to the asset commit to avoid stale cached images. After publishing new banners, update both image URLs in `profile/README.md` to that new asset commit and publish the README change. Generating assets does not publish changes by itself or change the GitHub organization avatar; avatar uploads are managed separately in organization settings.
