...
# One canonical master at every size; 4x supersampling smooths small icons.
# Reduce once with ImageMagick's alpha-aware resize; keep alpha enabled.
for size in 2048 1024 512 256 128 64 48 32 16; do
  ...
  rsvg-convert --width "$render_size" --height "$render_size" "$master" \
    --output "$build_dir/render.png"
  if (( size <= 64 )); then
    magick "$build_dir/render.png" -filter Lanczos \
      -resize "${size}x${size}" -strip "png/commonfire-logo-${size}.png"
  else
    magick "$build_dir/render.png" -strip "png/commonfire-logo-${size}.png"
  fi
  # Optional solid alternatives; primary exports remain full color at every size.
  if (( size < 48 )); then
    for ink in black white; do
      magick "png/commonfire-logo-${size}.png" -channel RGB -fill "$ink" \
        -colorize 100 +channel -strip "monochrome/commonfire-logo-${ink}-${size}.png"
      magick "monochrome/commonfire-logo-${ink}-${size}.png" -define webp:lossless=true \
        "monochrome/commonfire-logo-${ink}-${size}.webp"
    done
  fi
...
magick png/commonfire-logo-64.png png/commonfire-logo-48.png \
  monochrome/commonfire-logo-white-32.png monochrome/commonfire-logo-white-16.png \
  favicon/favicon-on-dark.ico
